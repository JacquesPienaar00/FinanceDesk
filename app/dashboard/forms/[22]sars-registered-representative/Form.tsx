...
const formSchema = z.object({
  ...
  representativePhotoWithId: z
    .any()
    .refine((files) => files?.length > 0, 'Photo with ID is required'),
  representativeProofOfAddress: z
    .any()
    ...
    try {
      ...
      Object.entries(data).forEach(([key, value]) => {
        if (
          ...
          key === 'representativeIdCopy' ||
          key === 'representativePhotoWithId' ||
          key === 'representativeProofOfAddress' ||
          key === 'certificateCopy'
        ) {
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="registrationDocuments">
                Registration Documents (CIPC - COR14.3) (Optional)
              </Label>
              <Input
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              {errors.representativeFullName && (
                <p className="text-sm text-red-500">{errors.representativeFullName.message}</p>
              )}
            </div>
            <div className="space-y-2">
              ...
              {errors.representativeSurname && (
                <p className="text-sm text-red-500">{errors.representativeSurname.message}</p>
              )}
            </div>
            <div className="space-y-2">
              ...
              {errors.representativeIdNumber && (
                <p className="text-sm text-red-500">{errors.representativeIdNumber.message}</p>
              )}
            </div>
            <div className="space-y-2">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select onValueChange={field.onChange} value={field.value}>
                    <SelectTrigger
                      className={errors.representativePosition ? 'border-red-500' : ''}
                    >
              ...
              {errors.representativePosition && (
                <p className="text-sm text-red-500">{errors.representativePosition.message}</p>
              )}
            </div>
            <div className="space-y-2">
              ...
              {errors.representativeIdCopy && (
                <p className="text-sm text-red-500">
                  {errors.representativeIdCopy.message as React.ReactNode}
                </p>
              )}
            </div>
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-500">
                Upload a photo of the person holding their ID and a sign that says &quot;update my
                details&quot;. Ensure the information on the ID is clear and visible.
              </p>
              {errors.representativePhotoWithId && (
                <p className="text-sm text-red-500">
                  {errors.representativePhotoWithId.message as React.ReactNode}
                </p>
              )}
            </div>
            <div className="space-y-2">
              ...
              {errors.representativeProofOfAddress && (
                <p className="text-sm text-red-500">
                  {errors.representativeProofOfAddress.message as React.ReactNode}
                </p>
              )}
            </div>
            <div className="space-y-2">
              ...
              />
              <p className="text-sm text-gray-500">
                As per share certificate or registration documents
              </p>
              {errors.dateOfAppointment && (
                <p className="text-sm text-red-500">
                  {errors.dateOfAppointment.message as React.ReactNode}
                </p>
              )}
            </div>
          </CardContent>
            ...
            {remainingDirectorsOption === 'uploadCertificate' && (
              <div className="space-y-2">
                ...
                {errors.certificateCopy && (
                  <p className="text-sm text-red-500">
                    {errors.certificateCopy.message as React.ReactNode}
                  </p>
                )}
              </div>
            )}
            ...
            {remainingDirectorsOption === 'enterDetails' && (
              <div className="space-y-4">
                  ...
                  <div key={field.id} className="space-y-4 rounded-md border p-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className={
                          errors.remainingDirectors?.[index]?.fullName ? 'border-red-500' : ''
                        }
                      />
                      {errors.remainingDirectors?.[index]?.fullName && (
                        <p className="text-sm text-red-500">
                          {errors.remainingDirectors[index]?.fullName?.message as React.ReactNode}
                        </p>
                      )}
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className={
                          errors.remainingDirectors?.[index]?.surname ? 'border-red-500' : ''
                        }
                      />
                      {errors.remainingDirectors?.[index]?.surname && (
                        <p className="text-sm text-red-500">
                          {errors.remainingDirectors[index]?.surname?.message as React.ReactNode}
                        </p>
                      )}
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className={
                          errors.remainingDirectors?.[index]?.idNumber ? 'border-red-500' : ''
                        }
                      />
                      {errors.remainingDirectors?.[index]?.idNumber && (
                        <p className="text-sm text-red-500">
                          {errors.remainingDirectors[index]?.idNumber?.message as React.ReactNode}
                        </p>
                      )}
                    </div>
                    {index > 0 && (
                ...
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => append({ fullName: '', surname: '', idNumber: '' })}
                >
